...
const FAQItem: React.FC<FAQItemProps> = ({ question, answer, isOpen, onToggle }) => {
  return (
    <div className="border-b border-gray-200 last:border-b-0">
      <button
        className="flex justify-between items-center w-full py-4 text-left focus:outline-none focus:ring-2 focus:ring-[#D0BFB4] focus:ring-opacity-50 rounded-lg transition-all duration-200 ease-in-out"
        onClick={onToggle}
      >
        <span className="font-medium text-base sm:text-lg text-gray-900 pr-4">{question}</span>
        <motion.div
          initial={false}
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="pb-4 text-gray-700 text-sm sm:text-base leading-relaxed">{answer}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  ...
  return (
    <section id="faq" className="py-16 sm:py-20">
      <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="font-inter text-4xl sm:text-5xl font-bold mb-2 text-center text-gray-900">FAQ</h2>
          <h3 className="font-poppins text-xl sm:text-2xl font-medium mb-8 text-center text-gray-500">Frequently Asked Questions</h3>
